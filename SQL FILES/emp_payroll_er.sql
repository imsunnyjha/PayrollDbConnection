...
AS
BEGIN
	INSERT INTO employee_payroll values(@name, @basic_pay,@start_date, @Gender, @phonenumber, @address, @department, @deductions, @taxable_pay, @income_tax, @net_pay)
END
GO

SELECT * FROM employee_payroll;

--CREATE TABLE SALARY INFO
...
CREATE TABLE employeeInfo(
    empId INT FOREIGN KEY REFERENCES salaryInfo(empId),
    empName VARCHAR(50) NOT NULL PRIMARY KEY,
    designation VARCHAR(50) NOT NULL
); 

--INSERT DATA INTO SALARY INFO TABLE
INSERT INTO salaryInfo(salaryId,salaryAmt,[month])
    VALUES (23,25000,'october'),(69,41000,'january'),(74,10000,'december');

--INSERT INTO EMPLOYEE INFO TABLE
INSERT INTO employeeInfo(empId,empName,designation)
    VALUES(1,'John','Manager'),(2,'Joe','Admin'),(3,'jasmine','account');

--VIEW THE TABLE
SELECT * FROM salaryInfo;    
SELECT * FROM employeeInfo;    
  
--CREATE TABLE FOR UC5
...
--INSERT VALUES
Insert Into empdetail(ename,address,dept,pay,sdate,edate)
	Values('Sunny','8/42 kayne rd','MD',20000,'2020-10-30','2020-06-21'),
        ('Arjun','8/85 lyson marg','audit',50000,'2020-09-05','2021-06-20');

--VIEW THE TABLE empdetail
SELECT * FROM empdetail;

DELETE FROM employee_payroll WHERE lower(name)='john';
SELECT * FROM employee_payroll;

--uc7 of emp payroll system
SELECT gender,COUNT(basic_pay) AS TotalCount,SUM(basic_pay) AS TotalSum, AVG(basic_pay) AS AverageValue, 
        MIN(basic_pay) AS MinValue, MAX(basic_pay) AS MaxValue
    FROM employee_payroll 
    WHERE gender = 'F' GROUP BY gender;

--uc8 of emp payroll system
